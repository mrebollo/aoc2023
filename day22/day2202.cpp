/**
 ...
 total de caídas cuando se eliminan bloques.
 Estrategia: 
 - obtener bloques no seguros
 - elininar bloque no seguro
 - dejar caer el resto de bloques que caen.
 - ¿si en un nivel no cae ninguno hace falta seguir? -> no
 **/
...
//TODO: no funciona en el caso general 
// porque el orden del conjunto y de inserción no es el mimsmo
int cascade(vector<vector<vector<int> > > &mat, set<block> &board, block b){
    int falls = 0;
    vector<block> removed;
    removed.push_back(b);
    removeblock(mat, b);
    set<block>::iterator bpos = board.upper_bound(b);
    while(bpos != board.end() && blockonair(mat, *bpos)){
        falls++;
        removed.push_back(*bpos);
        removeblock(mat, *bpos);
        bpos++;
    }
    for(auto rb: removed)
        insertblock(mat, rb);
...
set<block> unsafeblocks(vector<vector<vector<int> > > &mat, set<block> &board){
    set<block> unsafe;
    for(auto b: board){
        removeblock(mat, b);
        ...
        insertblock(mat, b);
        if (!stable) {
            cout << "block " << b.id << " is not safe" << endl;
            unsafe.insert(b);
        }
    }
    return unsafe;
...
int unsaferemoval(vector<vector<vector<int> > > &mat, set<block> &board, set<block> &unsafe){
    int totalfalls = 0;
    for(auto b: unsafe){
        int fall = cascade(mat, board, b);
        totalfalls += fall;
        cout << "block " << b.id << " falls " << fall << " total " << totalfalls << endl;
    }
    return totalfalls;
}
...
int main(){
    ...
    set<block> unsafe = unsafeblocks(matrix, board);
    cout << "unsafe blocks: " << unsafe.size() << endl;
    cout << "safe blocks: " << board.size() - unsafe.size() << endl;
    int totalfalls = unsaferemoval(matrix, board, unsafe);
    cout << "total falls: " << totalfalls << endl;
    return 0;
}